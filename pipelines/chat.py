...
class PipelineRequestError(Exception):
    ...
    def __init__(self, original: Exception, request: dict[str, Any]) -> None:
    ...
    def _format(self) -> str:
        lines = [
            f"chat.completions.create failed: {_provider_error_text(self.original)}",
            "",
...
class StructuredOutputError(PipelineRequestError):
    """Compatibility error type kept for the public API."""
    ...
    def _format(self) -> str:
        lines = [
            f"structured output failed after {self.retries} repair attempt(s): {self.original}",
            "",
            f"Request params: {_format_params(self.params)}",
            f"Messages ({len(self.messages)}):",
            *_format_messages(self.messages),
            "",
            "Model's last output:",
            *(f"  {line}" for line in (str(self.last_output).splitlines() or [""])),
            "",
            "Inspect the full request on the exception: .params, .messages, .last_output, .original",
        ]
        return "\n".join(lines)
...
def _request_context_note(request: dict[str, Any]) -> str:
...
class PipelinedChatCompletions:
    ...
    async def create(
        ...
        **chat_kwargs: Any,
        ...
        raise RuntimeError(f"Exceeded internal tool iteration safety limit: {self._MAX_TOOL_ITERATIONS}")
    ...
    async def _repair_structured_output(
        ...
        *,
        ...
        return self._construct_fallback(structured.model_cls, generation.text), last_response
    ...
    async def _generate_one(
        ...
        *,
        ...
        while True:
            ...
            if response_format is None and structured is not None:
            ...
            try:
                response = await self._call_create(current_kwargs)
                ...
                if response_format_type == "json_schema" and ("json_schema" in error_text or "response_format" in error_text):
                    ...
                    response = await self._call_create(fallback_kwargs)
    ...
    async def _call_create(self, kwargs: dict[str, Any]) -> Any:
        ...
        try:
            ...
            if _is_provider_error(exc):
                raise PipelineRequestError(exc, kwargs) from None
            if hasattr(exc, "add_note"):
                exc.add_note(_request_context_note(kwargs))
            raise
        return value
